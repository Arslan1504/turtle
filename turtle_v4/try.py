...
# ==========================================
LIDAR_PORT = '/dev/ttyUSB0'    # Updated for Raspberry Pi
LIDAR_BAUD = 230400     
OPENCR_PORT = '/dev/ttyACM0'   # Updated for Raspberry Pi
OPENCR_BAUD = 115200
WHEEL_RADIUS_CM = 3.3       
TRACK_WIDTH_CM = 17.6       
...
is_stopped_by_safety = False  # Global flag for LiDAR safety
...
# ==========================================
# 3. Background Threads (Vision, Odometry, LiDAR)
# ==========================================
latest_frame = None
...
def lidar_worker():
    global running, is_stopped_by_safety
    try:
        ...
        while running:
            ...
            while len(packet_buffer) >= 42:
                if packet_buffer[0] == 250 and 160 <= packet_buffer[1] <= 219:
                    ...
                    # Wait for a full 360 rotation to finish
                    if index == 219:
                        temp_obstacle = False
                        
                        # Check a 40-degree cone straight ahead
                        angles_to_check = list(range(0, 21)) + list(range(339, 360))
                        
                        for check_angle in angles_to_check:
                            dist = lidar_scan.get(check_angle, 9999)
                            if 0 < dist < 100:
                                temp_obstacle = True
                                break 
                        
                        # Update global flag safely
                        with lock:
                            is_stopped_by_safety = temp_obstacle
...
# ==========================================
def run_robot():
    global latest_frame, latest_detections, running, is_stopped_by_safety
    ...
    try:
        arduino = serial.Serial(OPENCR_PORT, OPENCR_BAUD, timeout=1)
        time.sleep(2)
    except Exception as e:
    ...
    try:
        while True:
            ...
            with lock:
                ...
                safety_stop_active = is_stopped_by_safety # Read the current LiDAR state
            ...
            if curr_time - last_serial_time >= 0.1:
                ...
                # Calculate what the AI WANTS to do
                ...
                # OVERRIDE the AI if the LiDAR sees an obstacle
                if safety_stop_active:
                    command = "0,0\n"
                    action_text = "BLOCKED BY LiDAR"
